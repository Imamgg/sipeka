            ...
            <div class="bg-gray-50 rounded-lg p-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="text-sm text-gray-900">
                        {{ $attendance->presence->classSchedule->teacher->user->name }}</div>

                    <div class="text-sm font-medium text-gray-500">Tanggal:</div>
                    <div class="text-sm text-gray-900">{{ $attendance->presence->date->format('d M Y') }}</div>
                    ...
                    <div class="text-sm">
                        ...
                        @elseif ($attendance->verification_status == 'verified')
                            <span
                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                Terverifikasi oleh {{ $attendance->verifier->name }} pada
                                {{ $attendance->verified_at->format('d M Y H:i') }}
                            </span>
                        @else
                            <span
                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                                Ditolak oleh {{ $attendance->verifier->name }} pada
                                {{ $attendance->verified_at->format('d M Y H:i') }}
                            </span>
                        @endif
                    </div>
