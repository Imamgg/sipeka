<x-teacher-layout>
        ...
        <div class="mb-8">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-gray-600">Ubah nilai untuk {{ $grade->student->user->name }}</p>
                </div>
            </div>
        </div>

        <form action="{{ route('teacher.grades.update', $grade) }}" method="POST" class="max-w-4xl">
            @csrf
            @method('PUT')

            <div class="p-6 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="text-sm font-semibold text-gray-900">{{ $grade->student->user->name }}</div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div class="text-sm text-gray-700">
                                    {{ $grade->student->classes->class_name ?? 'Belum Terdaftar' }}</div>
                            </div>
                            <div>
                ...
                <div>
                    ...
                    <select id="grade_type" name="grade_type" required
                        ...
                        <option value="tugas" {{ old('grade_type', $grade->grade_type) == 'tugas' ? 'selected' : '' }}>
                            Tugas</option>
                        <option value="kuis" {{ old('grade_type', $grade->grade_type) == 'kuis' ? 'selected' : '' }}>
                            Kuis</option>
                        <option value="uts" {{ old('grade_type', $grade->grade_type) == 'uts' ? 'selected' : '' }}>
                            UTS</option>
                        <option value="uas" {{ old('grade_type', $grade->grade_type) == 'uas' ? 'selected' : '' }}>
                            UAS</option>
                    </select>
                    @error('grade_type')
                ...
                <div>
                    ...
                    <input type="text" id="assessment_name" name="assessment_name"
                        value="{{ old('assessment_name', $grade->type_assessment) }}"
                ...
                <!-- Grade -->
                <div>
                    <label for="score" class="block text-sm font-semibold text-gray-700 mb-3">Nilai <span
                ...
                <!-- Notes -->
                <div>
                    <label for="description" class="block text-sm font-semibold text-gray-700 mb-3">Catatan</label>
                    <textarea id="description" name="description" rows="3" placeholder="Catatan tambahan untuk penilaian ini..."
                        class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $grade->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const gradeInput = document.getElementById('score');
