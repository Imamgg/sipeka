...
class AdminReportController extends Controller
{
    ...
    public function studentPerformance(Request $request)
    {
        ...
        $gradeDistribution = [
            'excellent' => $grades->where('score', '>=', 85)->count(),
            'good' => $grades->whereBetween('score', [70, 84])->count(),
            'average' => $grades->whereBetween('score', [60, 69])->count(),
            'poor' => $grades->where('score', '<', 60)->count(),
        ];
    ...
    public function teacherPerformance(Request $request)
    {
        ...
        $teacherPerformance = $teachers->map(function ($teacher) use ($selectedMonth, $selectedYear) {
            ...
            if (empty($classesNames)) {
                $classesNames = 'N/A';
            }      // Count materials created by teacher
            $materialsCount = $teacher->materials()
                ->whereMonth('created_at', $selectedMonth)
                ->whereYear('created_at', $selectedYear)
                ->count();

            // Get total materials ever created by teacher
            $totalMaterialsCount = $teacher->materials()->count();
            ...
            return [
                ...
                'activity_score' => $this->calculateActivityScore($gradeStats, $attendanceStats, $materialsCount)
            ];
        ...
        // Teacher statistics - using real data
        $totalMaterials = Material::count();
        $recentAttendances = Presence::whereMonth('date', $selectedMonth)
            ->whereYear('date', $selectedYear)
            ->get();
        $avgAttendanceRate = $recentAttendances->count() > 0 ?
            round(($recentAttendances->whereIn('status', ['present', 'late'])->count() / $recentAttendances->count()) * 100, 2) : 0;
        ...
        // Top teachers (top 5)
        $topTeachers = $teacherPerformance->take(5);

        // Recent teacher activities - using real data from materials uploads and grade entries
        $recentMaterials = Material::with(['teacher.user'])
            ->whereMonth('created_at', $selectedMonth)
            ->whereYear('created_at', $selectedYear)
            ->orderBy('created_at', 'desc')
            ->limit(5)
        ...
        $recentGrades = Grade::with(['teacher.user'])
            ->whereMonth('created_at', $selectedMonth)
            ->whereYear('created_at', $selectedYear)
            ->orderBy('created_at', 'desc')
            ->limit(3)
        ...
        // Add material upload activities
        foreach ($recentMaterials as $material) {
            $teacherActivities->push([
                'teacher' => $material->teacher->user->name ?? 'Unknown Teacher',
                'activity' => 'Uploaded material: ' . $material->title,
                'time' => $material->created_at->diffForHumans()
            ]);
        }

        // Add grade entry activities
        foreach ($recentGrades as $grade) {
            $teacherActivities->push([
                'teacher' => $grade->teacher->user->name ?? 'Unknown Teacher',
                'activity' => 'Added grade for ' . $grade->type_assessment,
                'time' => $grade->created_at->diffForHumans()
            ]);
        }

        // Sort by time and take only the most recent 5
        $teacherActivities = $teacherActivities->sortByDesc(function ($activity) {
     ...
     */
    private function calculateActivityScore($gradeStats, $attendanceStats, $materialsCount = 0)
    {
        ...
        $materialsScore = min($materialsCount * 5, 20); // Max 20 points for materials creation

        return round($gradeScore + $attendanceScore + $qualityScore + $materialsScore, 2);
    }
}
